...
def compute_rewards(sequences):
    ...
    for (seq, sim) in zip(sequences[:5], cosine_similarities[:5]):
        print(f"Sequence: {seq}, Cosine Similarity: {sim:.4f}")
    print('mean cosine similarity:', cosine_similarities.mean().item())
    print("-" * 100)
